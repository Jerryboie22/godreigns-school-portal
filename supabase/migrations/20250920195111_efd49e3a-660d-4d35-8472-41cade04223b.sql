-- Create missing tables to fix build errors (fixed version)
...
CREATE POLICY "Users can view own fees" ON public.school_fees FOR SELECT USING (
  student_profile_id = auth.uid() OR
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role IN ('super_admin', 'admin'))
);
...
CREATE INDEX idx_school_fees_student ON public.school_fees(student_profile_id);
